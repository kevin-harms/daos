...
def betterSkipIf(cond, message=None):
    print "betterSkipIf 1"
    def decorator(function):
        def wrapper(self, *args, **kwargs):
            print "betterSkipIf 2"
            if callable(cond):
                print "betterSkipIf 3"
                if cond(self):
                    print "betterSkipIf 4"
                    raise TestSkipError(message)
            else:
                print "betterSkipIf 5"
                if cond:
                    print "betterSkipIf 6"
                    raise TestSkipError(message)
        print "betterSkipIf 7"
        return wrapper
    print "betterSkipIf 8"
    return decorator
...
class Test(avocadoTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.log.info("__init__() executed from Apricot.Test")
...
class TestWithoutServers(Test):
    ...
    '''
    def setUp(self):
        self.log.info("setUp() executed from Apricot.TestWithoutServers")
    ...
    def tearDown(self):
        self.log.info("tearDown() executed from Apricot.TestWithoutServers")
...
class TestWithServers(TestWithoutServers):
    ...
    def tearDown(self):
        self.log.info("tearDown() executed from Apricot.TestWithServers")

        ServerUtils.stopServer(hosts=self.hostlist)

        # pylint: disable=no-member
        test_name = self._Test__name.str_filesystem()
        self.log.info("_Test__name: %s", test_name)
